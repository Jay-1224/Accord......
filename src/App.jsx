...
function AdminPanel({ knowledgeNotes, setKnowledgeNotes, conferences, setConferences, adminSchools, setAdminSchools, adminUsers, setAdminUsers, committees, setCommittees }) {
  ...
  const committeesCountFor = (confName) => committees.filter(c=>c.conference===confName).length;
  const delegatesCountFor  = (confName) => adminUsers.filter(u=>u.conference===confName && u.role==="delegate").length;
  const schoolsCountFor    = (confName) => (conferences.find(c=>c.name===confName)?.participatingSchools||[]).length;
        ...
        <div>
          ...
          {managedConf&&(
            <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.4)",zIndex:999,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
              <div style={{background:"#fff",borderRadius:12,padding:"28px 32px",width:520,maxWidth:"92vw",maxHeight:"85vh",overflowY:"auto",boxShadow:"0 16px 48px rgba(0,0,0,0.2)",fontFamily:"system-ui",boxSizing:"border-box"}}>
                ...
                <div style={{fontSize:11,color:C.textMuted,margin:"6px 0 18px"}}>Delegate counts update automatically as users register. Committees and participating schools below are set here.</div>

                <div style={{fontSize:11,fontWeight:700,color:C.textMuted,letterSpacing:0.6,marginBottom:10,marginTop:8}}>COMMITTEES</div>
                {committees.filter(c=>c.conference===managedConf.name).length===0&&<div style={{fontSize:12,color:C.textMuted,marginBottom:10}}>No committees added yet.</div>}
                {committees.filter(c=>c.conference===managedConf.name).map(c=>(
                  <div key={c.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"7px 0",borderBottom:`1px solid ${C.border}`}}>
                    <div><span style={{fontWeight:600,fontSize:13,color:C.navy}}>{c.name}</span><span style={{fontSize:12,color:C.textMuted,marginLeft:8}}>{c.fullName}</span></div>
                    <button onClick={()=>{setCommittees(cm=>cm.filter(x=>x.id!==c.id));logAction(`Committee removed from ${managedConf.name}: ${c.name}`,"conf");}} style={{...mkBtn("danger"),padding:"3px 9px",fontSize:11}}>✕</button>
                  </div>
                ))}
                <div style={{display:"flex",gap:6,marginTop:10,flexWrap:"wrap"}}>
                  <input value={newCommittee.name} onChange={e=>setNewCommittee(p=>({...p,name:e.target.value}))} placeholder="Short name (e.g. UNSC)" style={{...inputSt,flex:"1 1 130px"}}/>
                  <input value={newCommittee.fullName} onChange={e=>setNewCommittee(p=>({...p,fullName:e.target.value}))} placeholder="Full name" style={{...inputSt,flex:"1 1 160px"}}/>
                  <input value={newCommittee.topic} onChange={e=>setNewCommittee(p=>({...p,topic:e.target.value}))} placeholder="Agenda topic" style={{...inputSt,flex:"1 1 160px"}}/>
                  <button onClick={()=>{
                    if(!newCommittee.name.trim())return;
                    const id=newCommittee.name.trim().toLowerCase().replace(/[^a-z0-9]+/g,"-")+"-"+Date.now();
                    setCommittees(cm=>[...cm,{id,name:newCommittee.name.trim(),fullName:newCommittee.fullName.trim()||newCommittee.name.trim(),topic:newCommittee.topic.trim()||"To be announced",country:null,conference:managedConf.name}]);
                    logAction(`Committee added to ${managedConf.name}: ${newCommittee.name.trim()}`,"conf");
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6,maxHeight:180,overflowY:"auto",padding:"2px 2px"}}>
                  {adminSchools.map(s=>{
                    const checked = (managedConf.participatingSchools||[]).includes(s.name);
                    return (
                      <label key={s.id} style={{display:"flex",alignItems:"center",gap:8,fontSize:12.5,padding:"6px 8px",borderRadius:6,background:checked?C.navyLight:"transparent",cursor:"pointer"}}>
                        <input type="checkbox" checked={checked} onChange={()=>setManagedConf(p=>{
                          const cur=p.participatingSchools||[];
                          return {...p, participatingSchools: checked?cur.filter(n=>n!==s.name):[...cur,s.name]};
                        })}/>
                ...
                <div style={{display:"flex",gap:10,marginTop:20}}>
                  <button onClick={()=>{setManagedConf(null);setNewCommittee({name:"",fullName:"",topic:""});}} style={{...mkBtn(),flex:1}}>Cancel</button>
                  <button onClick={()=>{setConferences(cs=>cs.map(c=>c.id===managedConf.id?{...managedConf}:c));logAction(`Conference updated: ${managedConf.name}`,"conf");setManagedConf(null);setNewCommittee({name:"",fullName:"",topic:""});}} style={{...mkBtn("primary"),flex:1}}>Save Changes</button>
                </div>
              </div>
            </div>
          )}
      ...
      {adminTab==="schools"&&(
        <div>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16,flexWrap:"wrap",gap:10}}>
            <div><div style={{fontWeight:700,fontSize:16,color:C.navy}}>Schools</div><div style={{fontSize:12,color:C.textMuted,marginTop:2}}>A directory of schools on the platform, independent of any conference. Schools register for a specific conference themselves via "Bring Your Conference Online."</div></div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(180px,1fr))",gap:12}}>
            {visibleSchools.map(s=>(
              <div key={s.id} style={{...card,marginBottom:0}}>
                ...
                <div style={{fontSize:12,color:C.textMuted,marginBottom:12}}>{s.city}</div>
          ...
          {showNewSchool&&(
            <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.4)",zIndex:999,display:"flex",alignItems:"center",justifyContent:"center"}}>
              <div style={{background:"#fff",borderRadius:12,padding:"28px 32px",width:420,maxWidth:"92vw",boxShadow:"0 16px 48px rgba(0,0,0,0.2)",fontFamily:"system-ui"}}>
                ...
                {[{l:"School Name",k:"name",ph:"e.g. St. Xavier's School"},{l:"City",k:"city",ph:"e.g. Mumbai"}].map(f=>(
                ...
                <div style={{display:"flex",gap:10,marginTop:8}}>
                  <button onClick={()=>{setShowNewSchool(false);setNewSchool({name:"",city:""});}} style={{...mkBtn(),flex:1}}>Cancel</button>
                  <button onClick={()=>{if(!newSchool.name.trim()||!newSchool.city.trim())return;setAdminSchools(sc=>[...sc,{id:Date.now(),name:newSchool.name.trim(),city:newSchool.city.trim(),status:"pending"}]);logAction(`New school added: ${newSchool.name.trim()}`,"school");setNewSchool({name:"",city:""});setShowNewSchool(false);}} style={{...mkBtn("primary"),flex:1}}>Add School</button>
                </div>
              </div>
            </div>
          )}
...
function HomePage({ onEnter, onSignup, onParticipate }) {
  const FEATURES = [
  ...
  return (
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:"system-ui",color:C.text}}>
      ...
      <div style={{background:`linear-gradient(135deg,${C.navy} 0%,#1a3560 60%,${C.navy} 100%)`,padding:"70px 24px 80px",textAlign:"center"}}>
        <div style={{maxWidth:720,margin:"0 auto"}}>
          ...
          <div style={{display:"flex",gap:12,justifyContent:"center",flexWrap:"wrap"}}>
            <button onClick={onSignup} style={{...mkBtn("primary"),padding:"13px 26px",fontSize:15,fontWeight:600}}>Sign Up</button>
            <button onClick={onParticipate} style={{padding:"13px 26px",fontSize:15,fontWeight:600,borderRadius:6,border:"1px solid rgba(255,255,255,0.3)",background:"transparent",color:"#fff",cursor:"pointer",fontFamily:"system-ui"}}>Request to Participate</button>
          </div>
      ...
      <div style={{maxWidth:960,margin:"0 auto",padding:"64px 24px"}}>
        ...
        <div style={{fontSize:12,fontWeight:700,color:C.gold,letterSpacing:1.5,marginBottom:16,textAlign:"center"}}>GETTING STARTED</div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(260px,1fr))",gap:16,marginBottom:60}}>
          <div onClick={onSignup} style={{border:`1px solid ${C.border}`,borderRadius:12,padding:24,background:C.bgSoft,cursor:"pointer"}}>
            <div style={{fontSize:11,fontWeight:700,color:C.gold,letterSpacing:1,marginBottom:8}}>STEP 1</div>
            <div style={{fontWeight:700,fontSize:16,color:C.navy,marginBottom:8}}>Create your profile</div>
            <div style={{fontSize:13,color:C.textMuted,lineHeight:1.7}}>Sign up once — just your name, email, and password. No MUN or portfolio needed yet.</div>
          </div>
          <div onClick={onParticipate} style={{border:`1px solid ${C.border}`,borderRadius:12,padding:24,background:C.bgSoft,cursor:"pointer"}}>
            <div style={{fontSize:11,fontWeight:700,color:C.gold,letterSpacing:1,marginBottom:8}}>STEP 2</div>
            <div style={{fontWeight:700,fontSize:16,color:C.navy,marginBottom:8}}>Request to Participate</div>
            <div style={{fontSize:13,color:C.textMuted,lineHeight:1.7}}>Pick a conference and your school. If your school is registered for it, you're in — instantly.</div>
          </div>
        </div>
      ...
      <div style={{textAlign:"center",padding:"36px 24px 48px"}}>
        ...
        <div style={{display:"flex",gap:10,justifyContent:"center",flexWrap:"wrap"}}>
          <button onClick={onSignup} style={{...mkBtn("primary"),padding:"12px 28px",fontSize:14}}>Sign Up</button>
          <button onClick={onParticipate} style={{...mkBtn(),padding:"12px 28px",fontSize:14}}>Request to Participate</button>
  ...
  return (
    <div style={{minHeight:"100vh",background:"linear-gradient(135deg,#0f2044 0%,#1a3560 50%,#0f2044 100%)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:20,fontFamily:"system-ui",position:"relative"}}>
      ...
      <div style={{textAlign:"center",marginBottom:24}}>
        ...
        <div style={{fontSize:24,fontWeight:700,color:"#fff",fontFamily:"Georgia,serif"}}>Create your profile</div>
      ...
      <div style={{background:"#fff",borderRadius:14,padding:"28px 32px",width:"100%",maxWidth:400,boxShadow:"0 24px 60px rgba(0,0,0,0.3)",boxSizing:"border-box"}}>
        ...
        <div style={{fontSize:12.5,color:C.textMuted,marginTop:16,textAlign:"center"}}>Already have an account? <span onClick={onSigninClick} style={{color:C.navy,textDecoration:"underline",cursor:"pointer",fontWeight:600}}>Sign in</span></div>
      </div>
    </div>
  );
}

function LoginPage({ onLogin, onBack, adminUsers, onSignupClick }) {
  const [email, setEmail] = useState("");
  const [password, setPassword] = useState("");
  const [error, setError] = useState("");
  ...
  const handleLogin = (e) => {
    ...
    setTimeout(() => {
      const acc = adminUsers.find(a => a.email.trim().toLowerCase() === email.trim().toLowerCase() && a.password === password);
      if (acc) onLogin(acc);
  ...
  return (
    <div style={{minHeight:"100vh",background:"linear-gradient(135deg,#0f2044 0%,#1a3560 50%,#0f2044 100%)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:20,fontFamily:"system-ui",position:"relative"}}>
      ...
      <div style={{background:"#fff",borderRadius:14,padding:"32px 36px",width:"100%",maxWidth:420,boxShadow:"0 24px 60px rgba(0,0,0,0.3)"}}>
        ...
        <div style={{fontSize:13,color:C.textMuted,marginBottom:24}}>Enter credentials provided by your MUN secretariat, or <span onClick={onSignupClick} style={{color:C.navy,textDecoration:"underline",cursor:"pointer",fontWeight:600}}>sign up</span> if you're a new delegate.</div>
...
// ── Shared cross-user storage ──────────────────────────────────────────
// Inside Claude.ai, `window.storage` is provided automatically and just
// works. Once you publish this app yourself (GitHub Pages, Vercel, etc.),
// that API no longer exists — GitHub Pages only serves static files, it
// can't run a database. To keep "one shared conference list for everyone"
// working after you self-host, this app falls back to a free Firebase
// Realtime Database over plain fetch() (no SDK/install needed).
//
// To enable it:
//   1. Go to https://console.firebase.google.com → Add project (free tier).
//   2. Build → Realtime Database → Create Database → start in test mode.
//   3. Copy the URL shown at the top of the database page — it looks like
//      https://YOUR-PROJECT-ID-default-rtdb.firebaseio.com
//   4. Paste it below as FIREBASE_DB_URL.
// Until you do, the app still runs fine — it just won't sync between
// different visitors when self-hosted (same as before).
// Note: Firebase's default "test mode" rules allow anyone to read/write and
// expire after 30 days — fine for a class project/demo, but lock the rules
// down (require auth) before using this for anything sensitive.
const FIREBASE_DB_URL = "https://accord-bb357-default-rtdb.asia-southeast1.firebasedatabase.app";
const firebaseConfigured = () => typeof FIREBASE_DB_URL === "string" && !FIREBASE_DB_URL.includes("YOUR-PROJECT-ID");
const hasClaudeStorage = () => typeof window !== "undefined" && !!window.storage;
const sanitizeFbKey = (key) => key.replace(/[.#$[\]/\s]/g, "_");
async function fbGet(key) {
  const res = await fetch(`${FIREBASE_DB_URL}/${sanitizeFbKey(key)}.json`);
...
// by Firebase Realtime Database (see above) once self-hosted. Loads once on
// mount / whenever `key` changes, saves local changes back to whichever
// backend is available, and polls periodically so changes made by other
// people viewing the same published app are picked up here too.
function useSharedState(key, initialValue, pollMs=4000) {
  ...
  const remoteGet = async (k) => {
    if (hasClaudeStorage()) { const r = await window.storage.get(k, true); return r ? r.value : null; }
    const data = await fbGet(k);
    return (data===null || data===undefined) ? null : JSON.stringify(data);
  };
  const remoteSet = async (k, serialized, rawValue) => {
    if (hasClaudeStorage()) return window.storage.set(k, serialized, true);
    return fbSet(k, rawValue);
  };
  ...
  useEffect(() => {
    ...
    setSynced(false);
    (async () => {
      if (!key || !backendReady) { if(!cancelled) setSynced(true); return; }
      try {
        const remoteRaw = await remoteGet(key);
        if (!cancelled && remoteRaw !== null) {
          const remote = JSON.parse(remoteRaw);
          skipNextSave.current = true;
          lastSaved.current = remoteRaw;
          setValue(remote);
        }
      } catch { /* key doesn't exist yet, or backend unavailable — start fresh */ }
  ...
  useEffect(() => {
    if (!synced || !key || !backendReady) return;
  ...
  useEffect(() => {
    if (!key || !backendReady) return;
    const id = setInterval(async () => {
      try {
        const remoteRaw = await remoteGet(key);
        if (remoteRaw !== null && remoteRaw !== lastSaved.current) {
          const remote = JSON.parse(remoteRaw);
          lastSaved.current = remoteRaw;
          skipNextSave.current = true;
          setValue(remote);
...
export default function App() {
  ...
  // never by delegates.
  const [committees, setCommittees] = useSharedState("accord:committees", [
  ...
  const [adminSchools, setAdminSchools] = useSharedState("accord:admin-schools", [
    ...
    {id:3,name:"Bilaspur Public School",city:"Bilaspur",status:"pending"},
  ]);
  ...
  useEffect(()=>{
    if(role!=="host_school")return;
    const myConf = conferences.find(c=>c.name===user?.conference);
    if(myConf) setHostVenueDraft(myConf.venue);
  },[role, user?.conference]);
  useEffect(()=>{
  ...
  const startVotingBooth = () => {
    ...
    const committeeDelegates = adminUsers.filter(a=>a.role==="delegate" && a.committee===committee.name);
  ...
  // ── Sign-up: creates a bare platform profile only — no conference,
  // committee, or portfolio yet. That only ever gets set by Request to
  // Participate below, once a school match is approved.
  ...
  const tabs=[
    ...
    ...(role==="host_school"?[{id:"manage-conference",label:"Manage Conference"}]:[]),
  ...
  const renderDashboard=()=>{
    ...
    const dashDelegates = adminUsers.filter(a=>a.role==="delegate"&&a.conference===dashConference);
    const dashConfObj = conferences.find(c=>c.name===dashConference);
    const dashParticipatingSchools = dashConfObj?.participatingSchools || [];
    const committeeDelegates = adminUsers.filter(a=>a.role==="delegate"&&a.committee===userCommitteeName&&a.conference===user.conference);
    const committeeDais = adminUsers.filter(a=>(a.role==="chair"||a.role==="cochair")&&a.committee===userCommitteeName&&a.conference===user.conference);
    return(
    <div>
      ...
      {can(role,"manage_schools")&&(
        <div style={card}>
          <div style={cardTitle}>Participating Schools</div>
          {dashParticipatingSchools.length===0&&<div style={{fontSize:13,color:C.textMuted,padding:"8px 0"}}>No schools have been added to this conference yet.</div>}
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(160px,1fr))",gap:12}}>
            {dashParticipatingSchools.map(schoolName=>{
              const count = adminUsers.filter(a=>a.role==="delegate"&&a.conference===dashConference&&a.school===schoolName).length;
              return (<div key={schoolName} style={{background:C.bgSoft,border:`1px solid ${C.border}`,borderRadius:8,padding:14,minWidth:0}}><div style={{fontWeight:600,fontSize:12,color:C.navy,wordBreak:"break-word",lineHeight:1.4,marginBottom:4}}>{schoolName}</div><div style={{fontSize:11,color:C.textMuted}}>{count} delegate{count===1?"":"s"} registered</div></div>);
            })}
          </div>
        </div>
      )}
  ...
  const renderSpeakers=()=>{
    ...
    const DELEGATE_LIST = adminUsers.filter(a=>a.role==="delegate" && a.committee===committeeName && a.conference===committeeConference).map(a=>a.country);
    return(
      <div>
      {renderScopeSwitcher()}
      <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"1fr 1fr",gap:16}}>
  ...
  const renderScorecard=()=>{
    ...
    const allDelegates = adminUsers.filter(a => {
  ...
  const renderJurisdiction=()=>{
      ...
      return (
        <div>
          ...
          {committeesInScope.map(c=>{
            const members = adminUsers.filter(a=>a.committee===c.name && a.conference===scopeConference);
    ...
    const members = adminUsers.filter(a=>a.committee===userCommitteeName && a.conference===user.conference);
    const dais = members.filter(a=>a.role==="chair"||a.role==="cochair");
    const delegates = members.filter(a=>a.role==="delegate");
    return (
      <div style={card}>
  ...
  const renderManageConference=()=>{
    const myConf = conferences.find(c=>c.name===user.conference);
    if (!myConf) return <div style={{...card,textAlign:"center",padding:40,color:C.textMuted,fontSize:13}}>No conference is assigned to your account yet.</div>;
    const myCommittees = committees.filter(c=>c.conference===myConf.name);
    const participating = myConf.participatingSchools||[];
    return (
      <div>
        <div style={card}>
          <div style={cardTitle}>Conference Details</div>
          <div style={{marginBottom:12}}>
            <label style={{fontSize:11,fontWeight:600,color:C.textSec,display:"block",marginBottom:5,letterSpacing:0.5}}>NAME</label>
            <div style={{...inputSt,background:C.bgSoft,color:C.textMuted}}>{myConf.name}</div>
          </div>
          <div style={{marginBottom:4}}>
            <label style={{fontSize:11,fontWeight:600,color:C.textSec,display:"block",marginBottom:5,letterSpacing:0.5}}>VENUE</label>
            <input value={hostVenueDraft} onChange={e=>setHostVenueDraft(e.target.value)} style={inputSt}/>
          </div>
          <button onClick={()=>{setConferences(cs=>cs.map(c=>c.id===myConf.id?{...c,venue:hostVenueDraft}:c));}} style={{...mkBtn("primary"),marginTop:12,padding:"7px 16px",fontSize:12}}>Save Venue</button>
        </div>

        <div style={card}>
          <div style={cardTitle}>Committees</div>
          {myCommittees.length===0&&<div style={{fontSize:12,color:C.textMuted,marginBottom:10}}>No committees added yet.</div>}
          {myCommittees.map(c=>(
            <div key={c.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"7px 0",borderBottom:`1px solid ${C.border}`}}>
              <div><span style={{fontWeight:600,fontSize:13,color:C.navy}}>{c.name}</span><span style={{fontSize:12,color:C.textMuted,marginLeft:8}}>{c.fullName}</span></div>
              <button onClick={()=>setCommittees(cm=>cm.filter(x=>x.id!==c.id))} style={{...mkBtn("danger"),padding:"3px 9px",fontSize:11}}>✕</button>
            </div>
          ))}
          <div style={{display:"flex",gap:6,marginTop:10,flexWrap:"wrap"}}>
            <input value={hostNewCommittee.name} onChange={e=>setHostNewCommittee(p=>({...p,name:e.target.value}))} placeholder="Short name (e.g. UNSC)" style={{...inputSt,flex:"1 1 130px"}}/>
            <input value={hostNewCommittee.fullName} onChange={e=>setHostNewCommittee(p=>({...p,fullName:e.target.value}))} placeholder="Full name" style={{...inputSt,flex:"1 1 160px"}}/>
            <input value={hostNewCommittee.topic} onChange={e=>setHostNewCommittee(p=>({...p,topic:e.target.value}))} placeholder="Agenda topic" style={{...inputSt,flex:"1 1 160px"}}/>
            <button onClick={()=>{
              if(!hostNewCommittee.name.trim())return;
              const id=hostNewCommittee.name.trim().toLowerCase().replace(/[^a-z0-9]+/g,"-")+"-"+Date.now();
              setCommittees(cm=>[...cm,{id,name:hostNewCommittee.name.trim(),fullName:hostNewCommittee.fullName.trim()||hostNewCommittee.name.trim(),topic:hostNewCommittee.topic.trim()||"To be announced",country:null,conference:myConf.name}]);
              setHostNewCommittee({name:"",fullName:"",topic:""});
            }} style={{...mkBtn("primary"),padding:"8px 16px",fontSize:12,flex:"0 0 auto"}}>+ Add</button>
          </div>
        </div>
        ...
        <div style={card}>
          <div style={cardTitle}>Participating Schools</div>
          {adminSchools.length===0&&<div style={{fontSize:12,color:C.textMuted}}>No schools in the platform directory yet.</div>}
          <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"1fr 1fr",gap:6}}>
            {adminSchools.map(s=>{
              const checked = participating.includes(s.name);
              return (
                <label key={s.id} style={{display:"flex",alignItems:"center",gap:8,fontSize:12.5,padding:"6px 8px",borderRadius:6,background:checked?C.navyLight:"transparent",cursor:"pointer"}}>
                  <input type="checkbox" checked={checked} onChange={()=>{
                    setConferences(cs=>cs.map(c=>{
                      if(c.id!==myConf.id)return c;
                      const cur=c.participatingSchools||[];
                      return {...c, participatingSchools: checked?cur.filter(n=>n!==s.name):[...cur,s.name]};
                    }));
                  }}/>
                  <span style={{color:C.text}}>{s.name}</span>
                </label>
    ...
    <div style={{fontFamily:"'Georgia','Times New Roman',serif",minHeight:"100vh",background:C.bgSoft,color:C.text,display:"flex",flexDirection:"column",overflowX:"hidden",maxWidth:"100vw"}}>
      ...
      <div style={{display:"flex",flex:1,overflow:"hidden",position:"relative"}}>
        ...
        <div style={{flex:1,overflowY:"auto",overflowX:"hidden",padding:16,fontFamily:"system-ui",minWidth:0,boxSizing:"border-box"}}>
          ...
          {activeTab==="manage-conference" && (role==="host_school" ? renderManageConference() : <Lock reason="Host School access only."/>)}
          {activeTab==="alfred"    && renderAlfred()}
          {activeTab==="notes"     && renderNotes()}
          {activeTab==="clock"     && renderClock()}
          {activeTab==="admin"     && (can(role,"access_admin_panel") ? <AdminPanel knowledgeNotes={knowledgeNotes} setKnowledgeNotes={setKnowledgeNotes} conferences={conferences} setConferences={setConferences} adminSchools={adminSchools} setAdminSchools={setAdminSchools} adminUsers={adminUsers} setAdminUsers={setAdminUsers} committees={committees} setCommittees={setCommittees}/> : <Lock reason="Platform Admin access only."/>)}
        </div>
      </div>
    </div>
  );
