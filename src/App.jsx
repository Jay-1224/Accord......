...
// Unique Student IDs for the Master Conference Table — issued only by the
// School/Institution "Bring Your Conference Online" flow (the Creator side),
// and looked up (never issued) by the Participant login flow (the Validator side).
const generateStudentId = (existingIds=[]) => {
  let id;
  do { id = "ACD-" + String(Math.floor(10000 + Math.random()*90000)); } while (existingIds.includes(id));
  return id;
};
const DEMO_ACCOUNTS = [
...
function AdminPanel({ knowledgeNotes, setKnowledgeNotes, conferences, setConferences, adminSchools, setAdminSchools, masterTable, setMasterTable, schoolRegistrations, setSchoolRegistrations }) {
  ...
  const adminTabs = [{k:"overview",l:"Overview"},{k:"institutions",l:schoolRegistrations.length>0?`Institutions (${schoolRegistrations.length})`:"Institutions"},{k:"conferences",l:"Conferences"},{k:"users",l:"Users"},{k:"schools",l:"Schools"},{k:"knowledge",l:"Professional Notes"},{k:"activity",l:"Activity Log"}];
  ...
  return (
    <div>
      ...
      {adminTab==="institutions"&&(
        <div>
          <div style={{fontWeight:700,fontSize:16,color:C.navy,marginBottom:4}}>Institutions</div>
          <div style={{fontSize:12,color:C.textMuted,marginBottom:16}}>Schools that registered a conference through "Bring Your Conference Online." Student IDs are issued automatically at submission — this view is read-and-moderate only.</div>
          {schoolRegistrations.length===0&&<div style={{...card,textAlign:"center",padding:40,color:C.textMuted,fontSize:13}}>No institutions have registered yet.</div>}
          {schoolRegistrations.map(reg=>{
            const students = masterTable.filter(s=>s.school===reg.schoolName && s.conference===reg.conference);
            const isOpen = expandedRegId===reg.id;
            return (
              <div key={reg.id} style={{...card,marginBottom:12}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:12}}>
                  <div style={{minWidth:0}}>
                    <div style={{fontWeight:700,fontSize:14,color:C.navy}}>{reg.schoolName}</div>
                    <div style={{fontSize:12,color:C.textMuted,marginTop:4}}>{reg.contactName} · {reg.contactEmail}</div>
                    <div style={{fontSize:12,color:C.textMuted,marginTop:2}}>Hosting <b style={{color:C.navy}}>{reg.conference}</b> — {reg.venue}{reg.schedule?` · ${reg.schedule}`:""} · Code <span style={{fontFamily:"monospace",fontWeight:700,color:C.navy}}>{reg.munCode}</span></div>
                    <div style={{fontSize:11,color:C.textMuted,marginTop:6}}>{reg.studentCount} Student ID{reg.studentCount===1?"":"s"} issued · Submitted {reg.submittedAt}</div>
                  </div>
                  <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                    <button onClick={()=>setExpandedRegId(isOpen?null:reg.id)} style={{...mkBtn(),padding:"6px 14px",fontSize:12}}>{isOpen?"Hide":"View"} Student IDs</button>
                    <button onClick={()=>setSchoolRegistrations(rs=>rs.filter(x=>x.id!==reg.id))} style={{...mkBtn(),padding:"6px 10px",fontSize:12}}>✕</button>
                  </div>
                </div>
                {isOpen&&(
                  <div style={{marginTop:14,paddingTop:14,borderTop:`1px solid ${C.border}`}}>
                    {students.length===0&&<div style={{fontSize:12,color:C.textMuted}}>No Student ID records found for this registration.</div>}
                    {students.map(s=>(
                      <div key={s.studentId} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"7px 0",borderBottom:`1px solid ${C.border}`}}>
                        <div><span style={{fontFamily:"monospace",fontWeight:700,color:C.navy,fontSize:12}}>{s.studentId}</span><span style={{fontSize:12,color:C.textSec,marginLeft:10}}>{s.name}{s.country?` — ${s.country}`:""}</span></div>
                        <div style={{display:"flex",alignItems:"center",gap:8}}>
                          <span style={pill(s.status==="active"?"active":"locked")}>{s.status}</span>
                          <button onClick={()=>{setMasterTable(mt=>mt.map(x=>x.studentId===s.studentId?{...x,status:x.status==="active"?"suspended":"active"}:x));logAction(`Student ID ${s.status==="active"?"suspended":"restored"}: ${s.studentId}`,"user");}} style={{...mkBtn(s.status==="active"?"danger":"success"),padding:"3px 10px",fontSize:11}}>{s.status==="active"?"Suspend":"Restore"}</button>
                        </div>
                      </div>
                    ))}
                  </div>
...
function HomePage({ onEnter, onBringOnline, onJoinParticipant }) {
  const FEATURES = [
  ...
  return (
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:"system-ui",color:C.text}}>
      <div style={{position:"sticky",top:0,zIndex:50,background:"rgba(255,255,255,0.92)",backdropFilter:"blur(6px)",borderBottom:`1px solid ${C.border}`,padding:"14px 24px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        ...
        <button onClick={onEnter} style={{...mkBtn("primary"),padding:"9px 20px",fontSize:13}}>Sign In</button>
      </div>

      <div style={{background:`linear-gradient(135deg,${C.navy} 0%,#1a3560 60%,${C.navy} 100%)`,padding:"70px 24px 80px",textAlign:"center"}}>
        <div style={{maxWidth:720,margin:"0 auto"}}>
          <div style={{display:"inline-block",background:"rgba(255,255,255,0.1)",border:"1px solid rgba(255,255,255,0.2)",borderRadius:20,padding:"5px 14px",fontSize:12,color:"rgba(255,255,255,0.75)",marginBottom:20}}>Model United Nations, digitized</div>
          <div style={{fontSize:40,lineHeight:1.15,fontWeight:700,color:"#fff",fontFamily:"Georgia,serif",marginBottom:18}}>Run your MUN conference, beautifully.</div>
          <div style={{fontSize:16,color:"rgba(255,255,255,0.65)",lineHeight:1.7,marginBottom:32}}>Accord brings the Speakers List, Scorecards, committee chat, and an AI attaché into one platform — built for Secretariats, Chairs, and Delegates alike.</div>
          <div style={{display:"flex",gap:12,justifyContent:"center",flexWrap:"wrap"}}>
            <button onClick={onBringOnline} style={{...mkBtn("primary"),padding:"13px 26px",fontSize:15,fontWeight:600}}>Bring Your Conference Online</button>
            <button onClick={onJoinParticipant} style={{padding:"13px 26px",fontSize:15,fontWeight:600,borderRadius:6,border:"1px solid rgba(255,255,255,0.3)",background:"transparent",color:"#fff",cursor:"pointer",fontFamily:"system-ui"}}>Join as a Participant</button>
          </div>
          <div style={{marginTop:16,fontSize:12.5,color:"rgba(255,255,255,0.5)"}}>Already have credentials? <span onClick={onEnter} style={{color:"#fff",textDecoration:"underline",cursor:"pointer"}}>Sign in</span></div>
        </div>
      </div>
      ...
      <div style={{maxWidth:960,margin:"0 auto",padding:"64px 24px"}}>
        ...
        <div style={{fontSize:12,fontWeight:700,color:C.gold,letterSpacing:1.5,marginBottom:16,textAlign:"center"}}>TWO WAYS TO GET STARTED</div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(260px,1fr))",gap:16,marginBottom:60}}>
          <div onClick={onBringOnline} style={{border:`1px solid ${C.border}`,borderRadius:12,padding:24,background:C.bgSoft,cursor:"pointer"}}>
            <div style={{fontSize:11,fontWeight:700,color:C.gold,letterSpacing:1,marginBottom:8}}>FOR SCHOOLS</div>
            <div style={{fontWeight:700,fontSize:16,color:C.navy,marginBottom:8}}>Bring Your Conference Online</div>
            <div style={{fontSize:13,color:C.textMuted,lineHeight:1.7}}>Register your school's upcoming conference and your students. We generate a unique Student ID for each one, ready to hand out.</div>
          </div>
          <div onClick={onJoinParticipant} style={{border:`1px solid ${C.border}`,borderRadius:12,padding:24,background:C.bgSoft,cursor:"pointer"}}>
            <div style={{fontSize:11,fontWeight:700,color:C.gold,letterSpacing:1,marginBottom:8}}>FOR STUDENTS</div>
            <div style={{fontWeight:700,fontSize:16,color:C.navy,marginBottom:8}}>Join as a Participant</div>
            <div style={{fontSize:13,color:C.textMuted,lineHeight:1.7}}>Already have a Student ID from your school? Enter it to go straight into your MUN room and portfolio.</div>
          </div>
        </div>
      ...
      <div style={{textAlign:"center",padding:"36px 24px 48px"}}>
        <div style={{fontSize:16,fontWeight:700,color:C.navy,fontFamily:"Georgia,serif",marginBottom:16}}>Ready to bring your conference online?</div>
        <div style={{display:"flex",gap:10,justifyContent:"center",flexWrap:"wrap"}}>
          <button onClick={onBringOnline} style={{...mkBtn("primary"),padding:"12px 28px",fontSize:14}}>Bring Your Conference Online</button>
          <button onClick={onJoinParticipant} style={{...mkBtn(),padding:"12px 28px",fontSize:14}}>Join as a Participant</button>
          <button onClick={onEnter} style={{...mkBtn(),padding:"12px 28px",fontSize:14}}>Sign In</button>
        </div>
...
// ── B2B / INSTITUTION SIDE ──
// The School/Institution is the CREATOR of data: it registers a new
// conference and a roster of students. Submitting generates unique Student
// IDs written to the Master Conference Table — this component has no
// knowledge of, and shares no logic with, the Participant login below.
function SchoolOnboardingPage({ onSubmit, onBack }) {
  ...
  const [students, setStudents] = useState([{name:"",country:"",committee:""}]);
  const [error, setError] = useState("");
  const [submitting, setSubmitting] = useState(false);
  const [result, setResult] = useState(null);

  const updateStudent = (i,k,v) => setStudents(s=>s.map((row,idx)=>idx===i?{...row,[k]:v}:row));
  const addStudent = () => setStudents(s=>[...s,{name:"",country:"",committee:""}]);
  const removeStudent = (i) => setStudents(s=>s.length>1?s.filter((_,idx)=>idx!==i):s);

  const handleSubmit = () => {
    if (!schoolName.trim()||!contactName.trim()||!contactEmail.trim()) { setError("Please fill in your school and contact details."); return; }
    if (!confName.trim()||!venue.trim()) { setError("Please enter your conference name and venue."); return; }
    const validStudents = students.filter(s=>s.name.trim());
    if (validStudents.length===0) { setError("Add at least one student to register."); return; }
    setError(""); setSubmitting(true);
    setTimeout(()=>{
      const outcome = onSubmit({
        schoolName:schoolName.trim(), contactName:contactName.trim(), contactEmail:contactEmail.trim(),
        confName:confName.trim(), venue:venue.trim(), schedule:schedule.trim(),
        students: validStudents.map(s=>({name:s.name.trim(), country:s.country.trim(), committee:s.committee.trim()})),
      });
      setSubmitting(false); setResult(outcome);
    }, 500);
  };

  const copyAll = () => {
    if (!result) return;
    const text = result.students.map(s=>`${s.name}\t${s.studentId}`).join("\n");
    if (typeof navigator!=="undefined" && navigator.clipboard) navigator.clipboard.writeText(text).catch(()=>{});
  };

  if (result) {
    return (
      <div style={{minHeight:"100vh",background:"linear-gradient(135deg,#0f2044 0%,#1a3560 50%,#0f2044 100%)",padding:"40px 20px",fontFamily:"system-ui",display:"flex",justifyContent:"center"}}>
        <div style={{background:"#fff",borderRadius:14,padding:"32px 36px",width:"100%",maxWidth:560,boxShadow:"0 24px 60px rgba(0,0,0,0.3)",height:"fit-content"}}>
          <div style={{textAlign:"center",marginBottom:20}}>
            <div style={{fontSize:36,marginBottom:10}}>🎉</div>
            <div style={{fontWeight:700,fontSize:19,color:C.navy,fontFamily:"Georgia,serif"}}>{result.conference.name} is live</div>
            <div style={{fontSize:13,color:C.textMuted,marginTop:6}}>MUN Code <span style={{fontFamily:"monospace",fontWeight:700,color:C.navy}}>{result.conference.code}</span> · Share the Student IDs below with your students so they can log in.</div>
          </div>
          <div style={{border:`1px solid ${C.border}`,borderRadius:10,overflow:"hidden",marginBottom:16}}>
            <div style={{display:"flex",justifyContent:"space-between",background:C.bgSoft,padding:"8px 14px",fontSize:11,fontWeight:700,color:C.textMuted,letterSpacing:0.5}}><span>STUDENT</span><span>STUDENT ID</span></div>
            {result.students.map(s=>(
              <div key={s.studentId} style={{display:"flex",justifyContent:"space-between",padding:"9px 14px",borderTop:`1px solid ${C.border}`,fontSize:13}}>
                <span style={{color:C.text}}>{s.name}{s.country?` — ${s.country}`:""}</span>
                <span style={{fontFamily:"monospace",fontWeight:700,color:C.navy}}>{s.studentId}</span>
              </div>
            ))}
          </div>
          <div style={{display:"flex",gap:10}}>
            <button onClick={copyAll} style={{...mkBtn(),flex:1,padding:"11px"}}>Copy All IDs</button>
            <button onClick={onBack} style={{...mkBtn("primary"),flex:1,padding:"11px"}}>Back to Home</button>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div style={{minHeight:"100vh",background:"linear-gradient(135deg,#0f2044 0%,#1a3560 50%,#0f2044 100%)",padding:"40px 20px",fontFamily:"system-ui",display:"flex",justifyContent:"center",position:"relative"}}>
      {onBack&&<button onClick={onBack} style={{position:"absolute",top:20,left:20,background:"none",border:"none",color:"rgba(255,255,255,0.55)",fontSize:13,cursor:"pointer",display:"flex",alignItems:"center",gap:4}}>← Back to Home</button>}
      <div style={{width:"100%",maxWidth:560,height:"fit-content"}}>
        <div style={{textAlign:"center",marginBottom:22}}>
          <div style={{fontSize:24,fontWeight:700,color:"#fff",fontFamily:"Georgia,serif"}}>Bring Your Conference Online</div>
          <div style={{fontSize:13,color:"rgba(255,255,255,0.5)",marginTop:4}}>Register your school's conference and we'll generate Student IDs for your delegation.</div>
        </div>
        <div style={{background:"#fff",borderRadius:14,padding:"28px 32px",boxSizing:"border-box"}}>
          <div style={{fontSize:11,fontWeight:700,color:C.textMuted,letterSpacing:0.8,marginBottom:10}}>YOUR SCHOOL</div>
          {[{l:"School / Institution Name",v:schoolName,set:setSchoolName,ph:"e.g. St. Xavier's School"},{l:"Contact Person",v:contactName,set:setContactName,ph:"e.g. Ms. Priya Nair"},{l:"Contact Email",v:contactEmail,set:setContactEmail,ph:"you@school.edu"}].map(f=>(
            <div key={f.l} style={{marginBottom:12}}><label style={{fontSize:11,fontWeight:600,color:C.textSec,display:"block",marginBottom:5}}>{f.l}</label><input value={f.v} onChange={e=>{f.set(e.target.value);setError("");}} placeholder={f.ph} style={inputSt}/></div>
          ))}
          <div style={{fontSize:11,fontWeight:700,color:C.textMuted,letterSpacing:0.8,margin:"18px 0 10px"}}>YOUR CONFERENCE</div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12}}>
            <div><label style={{fontSize:11,fontWeight:600,color:C.textSec,display:"block",marginBottom:5}}>Conference Name</label><input value={confName} onChange={e=>{setConfName(e.target.value);setError("");}} placeholder="e.g. Xavier MUN 2026" style={inputSt}/></div>
            <div><label style={{fontSize:11,fontWeight:600,color:C.textSec,display:"block",marginBottom:5}}>Venue / City</label><input value={venue} onChange={e=>{setVenue(e.target.value);setError("");}} placeholder="e.g. Pune" style={inputSt}/></div>
          </div>
          <div style={{marginBottom:6}}><label style={{fontSize:11,fontWeight:600,color:C.textSec,display:"block",marginBottom:5}}>Schedule / Dates (optional)</label><input value={schedule} onChange={e=>setSchedule(e.target.value)} placeholder="e.g. 14–16 March 2026" style={inputSt}/></div>

          <div style={{fontSize:11,fontWeight:700,color:C.textMuted,letterSpacing:0.8,margin:"18px 0 10px"}}>YOUR STUDENTS</div>
          {students.map((s,i)=>(
            <div key={i} style={{display:"flex",gap:8,marginBottom:8,alignItems:"center"}}>
              <input value={s.name} onChange={e=>updateStudent(i,"name",e.target.value)} placeholder="Student name" style={{...inputSt,flex:2}}/>
              <input value={s.country} onChange={e=>updateStudent(i,"country",e.target.value)} placeholder="Portfolio (optional)" style={{...inputSt,flex:1}}/>
              <button onClick={()=>removeStudent(i)} style={{...mkBtn("danger"),padding:"9px 12px",flexShrink:0}}>✕</button>
            </div>
          ))}
          <button onClick={addStudent} style={{...mkBtn(),padding:"7px 14px",fontSize:12,marginBottom:16}}>+ Add Another Student</button>
          ...
          <button onClick={handleSubmit} disabled={submitting} style={{...mkBtn("primary"),width:"100%",padding:"12px",fontSize:14}}>{submitting?"Setting up your conference...":"Generate Student IDs"}</button>
        </div>
      </div>
    </div>
  );
...
// ── B2C / PARTICIPANT SIDE ──
// The Participant is the VALIDATOR: they hold a Student ID already issued
// by their school (above) and this page only ever looks that ID up against
// the Master Conference Table. It never creates or edits table data.
function ParticipantLoginPage({ onSubmit, onBack }) {
  ...
  const handleSubmit = (e) => {
    ...
    setTimeout(()=>{
      const outcome = onSubmit(studentId.trim());
      setLoading(false);
  ...
  return (
    <div style={{minHeight:"100vh",background:"linear-gradient(135deg,#0f2044 0%,#1a3560 50%,#0f2044 100%)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:20,fontFamily:"system-ui",position:"relative"}}>
      ...
      <div style={{textAlign:"center",marginBottom:26}}>
        ...
        <div style={{fontSize:24,fontWeight:700,color:"#fff",fontFamily:"Georgia,serif"}}>Participant Login</div>
        <div style={{fontSize:13,color:"rgba(255,255,255,0.5)",marginTop:4}}>Enter the Student ID your school gave you to join your MUN.</div>
      </div>
      <div style={{background:"#fff",borderRadius:14,padding:"32px 36px",width:"100%",maxWidth:400,boxShadow:"0 24px 60px rgba(0,0,0,0.3)",boxSizing:"border-box"}}>
        <label style={{fontSize:12,fontWeight:600,color:C.textSec,display:"block",marginBottom:8,letterSpacing:0.5}}>STUDENT ID</label>
        <input value={studentId} onChange={e=>{setStudentId(e.target.value);setError("");}} onKeyDown={e=>e.key==="Enter"&&handleSubmit()} placeholder="e.g. ACD-48213" style={{...inputSt,letterSpacing:2,fontWeight:700,fontFamily:"monospace",textAlign:"center",fontSize:16,textTransform:"uppercase"}} autoFocus/>
        {error&&<div style={{background:C.redBg,color:C.redTxt,borderRadius:7,padding:"10px 14px",fontSize:13,marginTop:14,border:"1px solid #fecaca"}}>{error}</div>}
        <button onClick={handleSubmit} disabled={loading} style={{...mkBtn("primary"),width:"100%",padding:"12px",fontSize:14,marginTop:16}}>{loading?"Checking...":"Enter My MUN"}</button>
        <div style={{fontSize:11.5,color:C.textMuted,marginTop:16,textAlign:"center",lineHeight:1.6}}>Don't have a Student ID? Ask your school's MUN coordinator — they registered your conference and received one for you.</div>
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  const [showSchoolOnboarding, setShowSchoolOnboarding] = useState(false);
  ...
  // Master Conference Table: the single source of truth the Participant flow
  // validates Student IDs against. Only ever written to by the School
  // onboarding flow (handleSchoolOnboarding below) — never by students.
  const [masterTable, setMasterTable] = useSharedState("accord:master-table", []);
  // Audit log of school "Bring Your Conference Online" submissions.
  const [schoolRegistrations, setSchoolRegistrations] = useSharedState("accord:school-registrations", []);
  ...
  // ── B2B Creator action: turns a school's submission into a new conference
  // plus fresh Master Conference Table rows. Nothing here is reachable from
  // the Participant side.
  const handleSchoolOnboarding = (data) => {
    const code = generateMunCode(conferences.map(c=>c.code));
    const newConf = { id: Date.now(), name: data.confName, venue: data.venue, status: "upcoming", code };
    setConferences(cs=>[...cs, newConf]);
    const existingIds = masterTable.map(s=>s.studentId);
    const generated = [];
    data.students.forEach(s=>{
      const studentId = generateStudentId([...existingIds, ...generated.map(g=>g.studentId)]);
      generated.push({
        studentId, name:s.name, country:s.country||null, committee:s.committee||null,
        conference:newConf.name, school:data.schoolName, status:"active",
        createdAt:new Date().toLocaleString([],{hour:"2-digit",minute:"2-digit",day:"2-digit",month:"short"}),
      });
    });
    setMasterTable(mt=>[...generated, ...mt]);
    setSchoolRegistrations(rs=>[{
      id:Date.now(), schoolName:data.schoolName, contactName:data.contactName, contactEmail:data.contactEmail,
      conference:newConf.name, munCode:code, venue:data.venue, schedule:data.schedule||null,
      studentCount:generated.length,
      submittedAt:new Date().toLocaleString([],{hour:"2-digit",minute:"2-digit",day:"2-digit",month:"short"}),
    }, ...rs]);
    return { conference:newConf, students:generated };
  };

  // ── B2C Validator action: only ever reads the Master Conference Table,
  // never writes to it.
  const handleParticipantLogin = (studentIdRaw) => {
    const sid = studentIdRaw.trim().toUpperCase();
    const record = masterTable.find(s=>s.studentId===sid);
    if (!record) return { ok:false, error:"We couldn't find that Student ID. Please check with your school coordinator." };
    setUser({
      email:`${record.studentId}@participant.accord`, name:record.name, role:"delegate",
      conference:record.conference, committee:record.committee||null, country:record.country||null,
      studentId:record.studentId,
    });
    setActiveTab("dashboard"); setHasEnteredMUN(false); setShowParticipantLogin(false);
    return { ok:true };
  };

  if (!user && showSchoolOnboarding) return <SchoolOnboardingPage onSubmit={handleSchoolOnboarding} onBack={()=>setShowSchoolOnboarding(false)}/>;
  if (!user && showParticipantLogin) return <ParticipantLoginPage onSubmit={handleParticipantLogin} onBack={()=>setShowParticipantLogin(false)}/>;
  if (!user && !showLogin) return <HomePage onEnter={()=>setShowLogin(true)} onBringOnline={()=>setShowSchoolOnboarding(true)} onJoinParticipant={()=>setShowParticipantLogin(true)}/>;
  if (!user) return <LoginPage onBack={()=>setShowLogin(false)} onLogin={u=>{setUser(u);setActiveTab("dashboard");setHasEnteredMUN(u.role==="admin");}}/>;
  ...
  const renderScopeSwitcher = () => {
    ...
    return (
      <div style={{display:"flex",gap:16,flexWrap:"wrap",marginBottom:16,padding:"12px 16px",background:C.bgSoft,border:`1px solid ${C.border}`,borderRadius:10,maxWidth:"100%",boxSizing:"border-box"}}>
        {isPlatformAdmin?(
    ...
    <div style={{fontFamily:"'Georgia','Times New Roman',serif",minHeight:"100vh",background:C.bgSoft,color:C.text,display:"flex",flexDirection:"column",overflowX:"hidden",maxWidth:"100vw"}}>
      <div style={{background:C.bg,borderBottom:`1px solid ${C.border}`,padding:isMobile?"0 10px":"0 16px",display:"flex",alignItems:"center",height:52,gap:isMobile?6:10,position:"sticky",top:0,zIndex:50}}>
        ...
        <div style={{marginLeft:"auto",display:"flex",alignItems:"center",gap:isMobile?6:8,fontFamily:"system-ui",flexShrink:0}}>
          ...
          <button onClick={()=>{setUser(null);setHasEnteredMUN(false);setProfileView(false);setShowLogin(false);setShowSchoolOnboarding(false);setShowParticipantLogin(false);}} style={{...mkBtn(),padding:isMobile?"4px 8px":"4px 10px",fontSize:11}}>{isMobile?"Out":"Sign out"}</button>
        </div>
      </div>
      <div style={{display:"flex",flex:1,overflow:"hidden",position:"relative"}}>
        ...
        <div style={{flex:1,overflowY:"auto",overflowX:"hidden",padding:16,fontFamily:"system-ui",minWidth:0,boxSizing:"border-box"}}>
          ...
          {activeTab==="admin"     && (can(role,"access_admin_panel") ? <AdminPanel knowledgeNotes={knowledgeNotes} setKnowledgeNotes={setKnowledgeNotes} conferences={conferences} setConferences={setConferences} adminSchools={adminSchools} setAdminSchools={setAdminSchools} masterTable={masterTable} setMasterTable={setMasterTable} schoolRegistrations={schoolRegistrations} setSchoolRegistrations={setSchoolRegistrations}/> : <Lock reason="Platform Admin access only."/>)}
        </div>
      </div>
    </div>
  );
