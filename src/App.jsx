...
function HomePage({ onEnter }) {
  const FEATURES = [
    { icon:"🎤", title:"Speakers List & Timers", text:"Run the General Speakers List with live speaking timers, queues, and yield handling — no more paper sign-up sheets." },
    { icon:"🗳️", title:"Live Voting Booths", text:"Chairs open Roll Call, Agenda, or Resolution votes on demand and record placards in real time." },
    ...
    { icon:"💬", title:"Committee Chat", text:"General and announcement channels per committee, with file and media attachments." },
    ...
    { icon:"🏛️", title:"Multi-Conference Admin", text:"Run several conferences from one dashboard, each with its own unique MUN code, schools, and roster." },
  ...
  const ROLES_INFO = [
    { role:"Platform Admin", text:"Creates and oversees every conference on Accord, manages schools and users platform-wide." },
    { role:"President & Secretary-General", text:"Full visibility across every committee, dais, and delegate in their conference." },
    { role:"Chair & Co-Chair", text:"Run the room — speakers list, voting, scorecards — for their own committee." },
    { role:"Delegate", text:"Track assignments, speak up, vote, and build a genuine record of performance." },
    { role:"Schools", text:"Register delegations and keep track of participation across conferences." },
  ];
  const ROADMAP = [
  ...
  return (
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:"system-ui",color:C.text}}>
      <div style={{position:"sticky",top:0,zIndex:50,background:"rgba(255,255,255,0.92)",backdropFilter:"blur(6px)",borderBottom:`1px solid ${C.border}`,padding:"14px 24px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        ...
        <button onClick={onEnter} style={{...mkBtn("primary"),padding:"9px 20px",fontSize:13}}>Sign In / Sign Up</button>
      </div>
      ...
      <div style={{background:`linear-gradient(135deg,${C.navy} 0%,#1a3560 60%,${C.navy} 100%)`,padding:"70px 24px 80px",textAlign:"center"}}>
        <div style={{maxWidth:720,margin:"0 auto"}}>
          ...
          <div style={{fontSize:16,color:"rgba(255,255,255,0.65)",lineHeight:1.7,marginBottom:32}}>Accord brings Speakers Lists, live Voting, Scorecards, committee chat, and an AI attaché into one platform — built for Secretariats, Chairs, and Delegates alike.</div>
          <button onClick={onEnter} style={{...mkBtn("primary"),padding:"13px 30px",fontSize:15,fontWeight:600}}>Get Started →</button>
        </div>
      </div>
      ...
      <div style={{textAlign:"center",padding:"36px 24px 48px"}}>
        ...
        <button onClick={onEnter} style={{...mkBtn("primary"),padding:"12px 28px",fontSize:14}}>Sign In / Sign Up</button>
...
export default function App() {
  ...
  if (!user && !showLogin) return <HomePage onEnter={()=>setShowLogin(true)}/>;
    ...
    <div style={{fontFamily:"'Georgia','Times New Roman',serif",minHeight:"100vh",background:C.bgSoft,color:C.text,display:"flex",flexDirection:"column",overflowX:"hidden",maxWidth:"100vw"}}>
      <div style={{background:C.bg,borderBottom:`1px solid ${C.border}`,padding:isMobile?"0 10px":"0 16px",display:"flex",alignItems:"center",height:52,gap:isMobile?6:10,position:"sticky",top:0,zIndex:50}}>
        ...
        <div style={{marginLeft:"auto",display:"flex",alignItems:"center",gap:isMobile?6:8,fontFamily:"system-ui",flexShrink:0}}>
          ...
          <button onClick={()=>{setUser(null);setHasEnteredMUN(false);setProfileView(false);setShowLogin(false);}} style={{...mkBtn(),padding:isMobile?"4px 8px":"4px 10px",fontSize:11}}>{isMobile?"Out":"Sign out"}</button>
        </div>
      </div>
